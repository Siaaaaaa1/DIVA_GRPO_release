...
class GeminiClient:
    def __init__(self):
        json_path = "/mmu_cd_ssd/zhangzhenyu06/workspace/gpt-o3/mmu-gemini-08579ef9be64.json"
    ...
    def request_to_azure_text_image(self,prompt_text, ground_truth, 
        ...
        try:
            ...
            if first_request:
                ...
                # 查找第一个<image>的位置
                split_index = input_text.find('<image>')

                if split_index == -1:  # 如果没有找到<image>
                    messages.append("Problem: " + input_text)
                    messages.append(Image.open(BytesIO(input_image)))
                else:  # 如果找到<image>
                    # 分割成三部分：<image>前的内容、<image>标记、<image>后的内容
                    before_image = "Problem: " + input_text[:split_index]
                    after_image = input_text[split_index + len('<image>'):]
                    
                    if before_image.strip():
                        messages.append(before_image)
                    messages.append(Image.open(BytesIO(input_image)))
                    if after_image.strip():
                        messages.append(after_image)
            else:
                messages = []
                messages.append(prompt_text)
                ...
                # 添加后续内容
                messages.append(answer_step_1)
                messages.append("The correct answer to this question is:" + ground_truth + "\n" + prompt_text2)
...
class Gpto3Client:
    ...
    def request_to_azure_text_image(self,prompt_text, ground_truth, 
        ...
        try:
            ...
            base64_image = base64.b64encode(input_image).decode('ascii')
            ...
            if first_request:
                ...
                messages = [
                    ...
                    {
                        "role": "assistant",
                        "content": answer_step_1
                    },
                    {
                        "role": "user",
                        "content": "The correct answer to this question is:" + ground_truth + "\n" + prompt_text2
                    }
                ]
...
def process_row(row: Dict[str, Any], gemini_client: GeminiClient, max_retries: int = 20) -> Dict[str, Any]:
...
def get_processed_ids(output_file: str) -> set:
    ...
    if os.path.exists(output_file):
        try:
            ...
            processed_ids = {r['id'] for r in results if 'status' in r and r['status']=="success"}
    ...
    if os.path.exists(output_file):
        try:
            with open(output_file, 'r', encoding='utf-8') as f:
                ...
                existing_results = [r for r in all_results if 'status' in r and r['status']=="success"]
